...
module.exports = {
	data: new SlashCommandBuilder()
		...
		.addStringOption((option) =>
			option
				.setName("message")
				.setDescription("Message to read out")
				.setRequired(true),
		)
		.addStringOption((option) =>
			option
		...
		.addNumberOption((option) =>
			option
				.setName("custom_pitch")
				.setDescription("Custom pitch level, overrides pitchName"),
		),
	async execute(interaction) {
		const message = interaction.options.getString("message");
		const pitch =
			pitches[interaction.options.getString("pitch_name")] ??
